...
const Sound = (() => {
  ...
  /* 딸깍 — 한 걸음 나아갔다는 짧은 신호 */
  /* pitch 는 음높이, vol 은 세기다. 여섯 게임이 같이 쓰므로
     한 게임만 크게 하고 싶을 때 vol 을 넘긴다. 안 넘기면 예전 그대로다. */
  function tick(pitch, vol) {
    tone(760 * (pitch || 1), 0.05, "square", vol == null ? 0.045 : vol);
  }
  ...
  const 볼륨표 = {
    ...
    bgm:     0.311,     // 0.22 에서 +3dB. 20%(+1.6dB)로는 귀에 차이가 안 왔다.
                       // 소리 크기에서 사람이 확실히 느끼는 최소 폭이 3dB 다
    oops:    0.55,     // 한 판에 여러 번 울린다. 성공음보다 확실히 아래로
  };
